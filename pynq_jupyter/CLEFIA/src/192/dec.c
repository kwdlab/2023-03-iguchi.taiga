...
void ClefiaGfn4Inv(unsigned char *y, const unsigned char *x, const unsigned char *rk, int r)
{
  ...
  rk += (r - 1) * 8;
  ByteCpy(fin, x, 16);
  ClefiaGfn4Inv_label5:while(r-- > 0){
    ClefiaF0Xor(fout + 0, fin + 0, rk + 0);
    ClefiaF1Xor(fout + 8, fin + 8, rk + 4);
    rk -= 8;
...
void ClefiaKeySet192(unsigned char *rk, const unsigned char *skey)
{
  ...
  rk += 8;
  ClefiaKeySet192_label9:for(i = 0; i < 11; i++){ /* round key (RKi (0 <= i < 44)) */
    if((i / 2) % 2){
...
void clefia_dec(const unsigned char ct[16], const unsigned char skey[32], unsigned char pt[16])
{
  unsigned char rk[8 * 22 + 16]; /* 8 bytes x 26 rounds(max) + whitening keys */
  int r=22;
...
int main(){
    const unsigned char ct[16] = {
        0xe2U,0x48U,0x2fU,0x64U,0x9fU,0x02U,0x8dU,0xc4U,
        0x80U,0xddU,0xa1U,0x84U,0xfdU,0xe1U,0x81U,0xadU
        };
